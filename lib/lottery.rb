...
class Lottery
  ...
  def register_weekdays csv_account_path, date_path
    ...
    csv_account.each do |account|
      date_arr.each do |date|
        ...
        register_unit account[:id], account[:pass], date, '17'
      end
    end
  end

  def register_saturday csv_account_path, date_path
    csv_account = CSVAccessor.read_account csv_account_path
    date_arr = File.open(date_path, :encoding => 'utf-8').read.split("\n")
    csv_account.each do |account|
      date = ordinal2date(account[:nth_sat], date_arr)
      puts "#{account[:name]}さんの土曜予約 #{date}, #{account[:opening_time]} を行います..."
      register_unit account[:id], account[:pass], date, account[:opening_time]
    end
  end

  private
  def register_unit id, pass, date, opening_time
    begin
      timeout(30) {
        LotteryAgent.new.register_unit(id, pass, date, opening_time)
      }
    rescue => ex
      puts ex.message
      retry
...
if $0 == __FILE__
  ...
  lottery = Lottery.new
  def lottery.register_unit id, pass, date, opening_time
    p [id, pass, date, opening_time]
  end
  lottery.register_weekdays(input_dir + "account.csv", input_dir + "date_weekdays.txt")
  print "\n\n\n\n"
  lottery.register_saturday(input_dir + "account.csv", input_dir + "date_saturday.txt")
end
